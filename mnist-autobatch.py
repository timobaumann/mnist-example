...
class MNISTClassify(object):
    def __init__(self, m):
        if args.conv:
            ...
            self.F1 = m.add_parameters((5, 5, 1, 8))
            self.b1 = m.add_parameters((8,))
            self.F2 = m.add_parameters((5, 5, 8, 32))
            self.b2 = m.add_parameters((32,))
            input_size = 7 * 7 * 32
        else:
            input_size = 28 * 28
    ...
    def __call__(self, x, dropout=False):
        if args.conv:
            ...
            x = dy.reshape(x, (7 * 7 * 32,))
        h1 = dy.tanh(self.W1 * x + self.h1bias)
        if dropout:
            h1 = dy.dropout(h1, DROPOUT_RATE)
        if HIDDEN_DIM2:
            h2 = dy.tanh(self.W2 * h1 + self.h2bias)
            if dropout:
                h2 = dy.dropout(h2, DROPOUT_RATE)
        logits = self.W3 * (h2 if HIDDEN_DIM2 else h1)
        return logits
